...
teacher_pipe = StableDiffusionPipeline.from_pretrained(
    teacher_model_id,
    torch_dtype=torch.float16,   # ⟵ добавили
    safety_checker=None
).to(device)
teacher_pipe.enable_attention_slicing()  # reduce memory usage for attention
teacher_pipe.vae.half()
teacher_unet = teacher_pipe.unet  # diffusion model (U-Net)
teacher_vae = teacher_pipe.vae    # VAE for encoding/decoding images
teacher_text_encoder = teacher_pipe.text_encoder  # CLIP text encoder
tokenizer = teacher_pipe.tokenizer                # tokenizer for captions
# Set teacher models to eval mode (we won't train them)
teacher_unet.eval()
teacher_vae.eval()
...
def create_student_model():
    ...
    student_channels = [256, 512, 512]  # Reduced channels for down blocks
    ...
    student_config = {
        "sample_size": 8,                 # latent spatial size for 64x64 images
        "in_channels": 4,                 # latent channels (4)
        "out_channels": 4,                # model predicts noise in latent space (4 channels)
        "down_block_types": student_down_blocks,
        "up_block_types": student_up_blocks,
        "block_out_channels": student_channels,
        "layers_per_block": 2,            # same as teacher
        "cross_attention_dim": 768,       # CLIP text embedding dimension
        "attention_head_dim": 8,          # same head dim as teacher
    }
...
# Create student models for comparison: one random, one initialized from teacher
student_unet_random = create_student_model()
student_unet_custom = create_student_model()
# Load teacher UNet in full precision on CPU for weight extraction
...
# Move student models to training device and set to train mode
...
# Set up diffusion noise scheduler for training (DDPM scheduler)
...
def train_student(student_unet, dataloader, optimizer, scheduler, out_dir, epochs=20, log_interval=250):
    ...
    # FID: InceptionV3 ожидает 299×299 и пиксели в [0..1]
    ...
    student_unet.train()
    global_step = 0
    # Pre-compute unconditional text embeddings for classifier-free guidance (empty caption)
    ...
    for epoch in range(epochs):
        for batch in dataloader:
            ...
            # Resize and normalize images to 64x64 for student
            ...
            # Convert images to tensor and normalize to [-1, 1] for VAE
            ...
            with torch.no_grad():
                latents_dist = teacher_vae.encode(image_tensors.half())  # encode in fp16
                latents = latents_dist.latent_dist.mean * 0.18215 # scaled latent
                latents = latents.float()  # convert to float32 for training

            # Sample random diffusion timesteps for each image in the batch
            B = latents.size(0)
